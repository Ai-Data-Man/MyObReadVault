---
{"dg-publish":true,"permalink":"/Logseq元问题库/pages/？ ElasticSearch 读写与查询数据的工作原理/","dgPassFrontmatter":true}
---
...
function iter(block, level) {
	...
	if (level > 0 && (block.text.startsWith("[[？") && block.text.endsWith("]]")) || (block.text.startsWith("#") && !block.text.contains(" "))) {
		return
	}
...
for (let pageLink of curPage.file.inlinks.sort()) {
	let page = dv.page(pageLink.path)
	let blocks = page.file.lists
	for(let block of blocks) {
		let blockText = block.text
		if (blockText == "[[" + curPage.file.name + "]]") {
			iter(block, 0)
		}
	}
}
