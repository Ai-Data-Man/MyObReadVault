---
{"dg-publish":true,"permalink":"/Logseq元问题库/pages/？ 消息队列 消息可靠性 设计/","dgPassFrontmatter":true}
---

奥义就是
	#消息队列 的ACK机制 + 生产者重试保证必达
	[[Logseq元问题库/pages/？ 消息队列 持久化 设计\|？ 消息队列 持久化 设计]]保证消息必存
	消费者消费ACK + #消息队列 的ACK机制保证必用
>
