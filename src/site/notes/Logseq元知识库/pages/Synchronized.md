---
{"Aliases":["#Synchronized"],"dg-publish":true,"permalink":"/Logseq元知识库/pages/Synchronized/","dgPassFrontmatter":true}
---
...
function iter(block, level) {
	...
	if (!blockObj["symbol"]) {
		...
		if (level != 0) {
			blockObj["text"] = blockObj["text"] + 
			"<mark style=\"background: #BBFABBA6;\">" + 
			block.link + "<\/mark>" 
		} 
		blockObj.symbol = "▬ ".repeat(level)
	}
	if (level > 0 && (block.text.startsWith("[[") && block.text.endsWith("]]"))) {
		return
	}
